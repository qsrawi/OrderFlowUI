...
export class SupplierHttpReqService {
  ...
  getCashDetailsBySupplier(supplierId: number): Observable<CashDto[]> {
    return this.http.get<CashDto[]>(`${this.supplierApiUrl}/GetCashDetailsBySupplier/${supplierId}`);
  }

  addReceipt(receipt: CreateReceiptDto): Observable<ReceiptDto> {
    return this.http.post<ReceiptDto>(`${this.supplierApiUrl}/AddReceipt`, receipt);
  }
